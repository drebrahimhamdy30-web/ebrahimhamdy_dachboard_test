function injectNavbar() {
  ...
  // ===== تقييد المحاسب =====
  var accountantPages = [
    'bank_monitor.html', 'sms.html', 'machine_import.html',
  ...
  if (userRoleEarly === 'accountant') {
    ...
    if (navBarAcc) navBarAcc.innerHTML =
      ...
      '<nav class="nav-links">' +
        ...
        '<a href="sms.html" class="'+(currentPage==='sms.html'?'active':'')+'">' +
...
var navContent =
  ...
  // Dropdown: فيزا — الأدمن والمحاسب يشوفوا الكل، الباقي فقط المحافظ والتسوية
  var isAccountantOrAdmin = isAdmin || userRole === 'manager' || userRole === 'accountant';
  var visaMenuItems = isAccountantOrAdmin ? [
  ...
  createFloatingDropdown('visa-dropdown', visaMenuItems);
