function injectNavbar() {
  ...
  var navContent =
    ...
    '<nav class="nav-links">' +
      ...
      // خدمة العملاء dropdown
      '<div id="cs-wrap" style="position:relative;display:flex;align-items:center;">' +
        ...
        '</a>' +
        '<div id="cs-dropdown" style="display:none;position:absolute;top:calc(100% + 6px);right:0;' +
          'background:#1e293b;border:1px solid #334155;border-radius:10px;min-width:200px;' +
          'box-shadow:0 8px 24px rgba(0,0,0,0.5);z-index:9999;overflow:hidden;">' +
          '<a href="customer_service.html" style="display:flex;align-items:center;gap:8px;' +
            'padding:11px 16px;text-decoration:none;font-size:0.82rem;border-bottom:1px solid #334155;' +
            'color:' + (currentPage === 'customer_service.html' ? '#38bdf8' : '#94a3b8') + ';' +
            'background:' + (currentPage === 'customer_service.html' ? '#0c4a6e' : 'transparent') + ';">' +
            '<i class="fas fa-headset"></i> خدمة العملاء</a>' +
          '<a href="shortages.html" style="display:flex;align-items:center;gap:8px;' +
            'padding:11px 16px;text-decoration:none;font-size:0.82rem;' +
            'color:' + (currentPage === 'shortages.html' ? '#38bdf8' : '#94a3b8') + ';' +
            'background:' + (currentPage === 'shortages.html' ? '#0c4a6e' : 'transparent') + ';">' +
            '<i class="fas fa-exclamation-triangle"></i> نواقص</a>' +
        '</div>' +
      '</div>' +
      ...
      // تعاقدات dropdown
      '<div id="contracts-wrap" style="position:relative;display:flex;align-items:center;">' +
        '<a id="contracts-toggle" href="#" style="' +
          ...
          '<i class="fas fa-chevron-down" style="font-size:0.6rem;margin-right:2px;"></i>' +
        '</a>' +
        '<div id="contracts-dropdown" style="display:none;position:absolute;top:calc(100% + 6px);right:0;' +
          'background:#1e293b;border:1px solid #334155;border-radius:10px;min-width:210px;' +
          'box-shadow:0 8px 24px rgba(0,0,0,0.5);z-index:9999;overflow:hidden;">' +
          '<a href="sales_contracts.html" style="display:flex;align-items:center;gap:8px;' +
            'padding:11px 16px;text-decoration:none;font-size:0.82rem;border-bottom:1px solid #334155;' +
            'color:' + (currentPage === 'sales_contracts.html' ? '#38bdf8' : '#94a3b8') + ';' +
            'background:' + (currentPage === 'sales_contracts.html' ? '#0c4a6e' : 'transparent') + ';">' +
            '<i class="fas fa-file-invoice"></i> فواتير التعاقد</a>' +
          '<a href="claims.html" style="display:flex;align-items:center;gap:8px;' +
            'padding:11px 16px;text-decoration:none;font-size:0.82rem;border-bottom:1px solid #334155;' +
            'color:' + (currentPage === 'claims.html' ? '#38bdf8' : '#94a3b8') + ';' +
            'background:' + (currentPage === 'claims.html' ? '#0c4a6e' : 'transparent') + ';">' +
            '<i class="fas fa-file-invoice-dollar"></i> مطالبات</a>' +
          '<a href="contracts.html" style="display:flex;align-items:center;gap:8px;' +
            'padding:11px 16px;text-decoration:none;font-size:0.82rem;' +
            'color:' + (currentPage === 'contracts.html' ? '#38bdf8' : '#94a3b8') + ';' +
            'background:' + (currentPage === 'contracts.html' ? '#0c4a6e' : 'transparent') + ';">' +
            '<i class="fas fa-file-contract"></i> لنا فواتير لدى العملاء</a>' +
        '</div>' +
      '</div>' +
  ...
  if (navBar) {
    ...
    // ربط الـ events بعد الـ inject مباشرة
    var csToggle = document.getElementById('cs-toggle');
    var contractsToggle = document.getElementById('contracts-toggle');

    if (csToggle) {
      csToggle.addEventListener('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        var menu = document.getElementById('cs-dropdown');
        var contracts = document.getElementById('contracts-dropdown');
    ...
    if (contractsToggle) {
      contractsToggle.addEventListener('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        var menu = document.getElementById('contracts-dropdown');
        var cs   = document.getElementById('cs-dropdown');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.addEventListener('click', function() {
    var m1 = document.getElementById('cs-dropdown');
    var m2 = document.getElementById('contracts-dropdown');
