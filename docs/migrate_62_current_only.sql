-- ═══════════════════════════════════════════════════════════════════
-- تتبع التحويلات: الكروت للتحويلات الجارية فقط
-- ═══════════════════════════════════════════════════════════════════
-- migrate_61 كان بيضيف كمان تحويلات النهاردة اللي وصلت أو اتلغت، عشان
-- خطوة «وصلت» تبان في الكارت. المالك طلب الجاري فقط: التحويلة تختفي من
-- الشاشة أول ما تخلص (توصل أو تتلغي)، فالشاشة تفضل مختصرة على اللي
-- محتاج متابعة.
--
-- الباقي زي ما هو: مراحل التحويلة في الكارت (إنشاء/تعيين/استلام/وصول)
-- وعدّادات الأصناف المعلّقة لكل فرع. مفيش أي تخزين.
-- ═══════════════════════════════════════════════════════════════════
...
ship AS (
  SELECT o.id, o.bill_no, o.branch_id, o.customer_name, o.cust_region, o.notes,
   ...
   WHERE (o.bill_no LIKE 'TRF-%' OR o.staff_notes = '📌 تحويلة')
     -- الجاري فقط: اللي وصلت أو اتلغت بتختفي من الشاشة
...
COMMENT ON FUNCTION public.get_open_transfers() IS
  'كروت تتبع التحويلات الجارية فقط (إنشاء/تعيين/استلام/وصول) + عدّادات الأصناف المعلّقة لكل فرع';
