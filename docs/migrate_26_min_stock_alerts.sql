...
-- ── بحث ذكي بالاسم لشاشة الحد الأدنى (2026-09-17) ──────────────────
-- search_stock_items(q, limit): بيدوّر في stock_flat بالكود أو الاسم.
-- ذكي إزاي: ar_norm بيشيل الهمزات والتشكيل (n_norm مخزّن ومعمول له فهرس
-- trgm)، وترتيب الكلمات مايفرقش (أطول كلمة هي المرشّح + تقييم بباقي
-- الكلمات)، وبيسامح غلطة إملائية بـsimilarity على الاسم وعلى أول كلمة
-- (n_fw)، والأصناف اللي عليها رصيد بتاخد نقط زيادة فتطلع فوق.
-- ⚠️ الشروط لازم تفضل على n_norm/n_fw (فيهم فهارس) — أول نسخة كانت
--    بتنادي ar_norm(n) في WHERE فكانت بتمسح 28 ألف صف (1.3 ثانية/بحث)،
--    وبعد التعديل بقت 12–50 مللي.
--    النص الكامل: SELECT pg_get_functiondef('public.search_stock_items(text,int)'::regprocedure);
