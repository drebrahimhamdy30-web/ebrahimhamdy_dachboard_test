-- ═══════════════════════════════════════════════════════════════════
-- كارت التحويلة: عدد الطلبات اللي في نفس الرحلة + استبعاد delivered
-- ═══════════════════════════════════════════════════════════════════
-- ① المالك عايز يعرف الطيار ماشي بإيه غير التحويلة: بنرجّع مع كل تحويلة
--    باقي طلبات نفس الرحلة (العدد + قايمة مختصرة) عشان الكارت يعرضها.
-- ② حالة 'delivered' مكانتش مستبعدة في migrate_62 — بس 'completed' —
--    فتحويلة اتسلّمت بالحالة دي كانت بتفضل في الشاشة كأنها جارية.
--    دلوقتي الحالات المنتهية: completed / delivered / cancelled / failed.
--
...
ship AS (
  SELECT o.id, o.bill_no, o.branch_id, o.customer_name, o.cust_region, o.notes,
   ...
   WHERE (o.bill_no LIKE 'TRF-%' OR o.staff_notes = '📌 تحويلة')
     -- الجاري فقط (delivered برضه حالة منتهية)
     AND o.status NOT IN ('completed','delivered','cancelled','failed')
...
by_branch AS (
  SELECT b.name,
         count(*) FILTER (WHERE p.req_b  = b.name) AS requested,
         count(*) FILTER (WHERE p.from_b = b.name) AS required
    FROM branches b
...
SELECT jsonb_build_object(
  ...
  'shipments', coalesce((
    SELECT jsonb_agg(jsonb_build_object(
             'id', sh.id, 'bill_no', sh.bill_no, 'status', sh.status, 'live', true,
             ...
             -- باقي طلبات نفس الرحلة: الطيار ماشي بإيه غير التحويلة
             'others', coalesce((
                SELECT jsonb_agg(jsonb_build_object(
                         'bill_no', o2.bill_no,
                         'name',    o2.customer_name,
                         'region',  o2.cust_region,
                         'status',  o2.status,
                         'is_transfer', (o2.bill_no LIKE 'TRF-%')) ORDER BY o2.bill_no)
...
COMMENT ON FUNCTION public.get_open_transfers() IS
  'كروت تتبع التحويلات الجارية فقط (إنشاء/تعيين/استلام/وصول) + باقي طلبات الرحلة + عدّادات الأصناف المعلّقة لكل فرع';
