...
## مراجعة المعمارية — الخطوة 7: الألوان بـtokens  ✅ 2026-09-04

**ملف جديد:** `tokens.css` (لوحة الألوان الموحّدة، فاتح + داكن)

### الضرر المقاس قبل التعديل
| | |
|---|---|
| شاشات **الوضع الداكن مابيأثّرش فيها خالص** | **6** (`shift_close`، `shift_history`، `pos_methods`، `machine_import`، `print_invoice`، `supplier_balances`) — كل واحدة معرّفة `--bg` محليًا بقيمة فاتحة ومفيهاش أي كتلة داكنة |
| شاشات فيها **بقع فاتحة** على خلفية داكنة | 36 |
| **وضعان داكنان مستقلان** بمفتاحين مختلفين | `phalix_theme` (الشل) و `phalix_dark` (التوزيع) — تفتح الداكن في واحد مايتغيّرش في التاني |
| ملفات بتعرّف نفس الـtokens بقيم مختلفة | 2 (`style.css` و `delivery/design-system.css`) |
| ألوان مكتوبة صريح | 2562 |

### اللي اتعمل
1. **`tokens.css`** — لوحة واحدة بتلات محدّدات للوضع الداكن:
   `html[data-theme="dark"]` + `body.dark`/`.dark` (توافق) + `prefers-color-scheme`.
2. **`style.css`** مابقاش يعرّف ألوان (كان بيتحمّل بعد tokens.css فبيلغيه).
   `delivery/design-system.css` اتساب زي ما هو — مبني صح أصلًا بفاتح وداكن.
3. **87 تعريف محلي مكرر** اتشال من 10 صفحات، و**31 منهم اترجّعوا** لأن قيمهم
   كانت **مختلفة عن قصد** (درجات أغمق للتباين). الوضع الداكن لسه بيتجاوزهم
   لأن `html[data-theme="dark"]` محدّده أقوى من `:root`.
4. **139 خلفية بيضا** مكتوبة صريح بقت `var(--surface)`.
5. **الزيبرا ورأس الجدول والهوفر** بقى ليهم tokens (`--zebra` / `--thead` /
   `--row-hover`) — دول لوحدهم كانوا **920 بقعة** في `medicine_orders`.
6. **الوضع الداكن اتوحّد**: مفتاح واحد، `data-theme` على `<html>`، والشل
   بيسرّيه للـiframe مع **كل** تحميل صفحة (كان بيضيع مع أول تنقّل).

### النتيجة المقاسة
| | قبل | بعد |
|---|---|---|
| الوضع الداكن بيأثّر في الصفحة | 47/53 | **53/53** |
| الوضع الفاتح مطابق للأصل | — | **53/53** |
| مفاتيح الوضع الداكن | 2 | **1** |
| خلفيات بيضا صريحة | 141 | **2** |
| بقع فاتحة في الوضع الداكن | — | 411 (كانت 1312 قبل tokens الزيبرا) |

### طريقة التحقق
شغّلت نسخة `git HEAD` والنسخة المعدّلة على **نفس الأصل** (`/base` و`/cur`)،
وقارنت **الخلفية واللون والحدود لكل عنصر** في 53 صفحة. الطريقة دي لقطت
3 مشاكل حقيقية ما كانتش هتبان بأي طريقة تانية:
1. `style.css` بيتحمّل بعد `tokens.css` فبيلغيه.
2. قيمه لـ`--success`/`--danger`/`--warning`/`--text-secondary` والظلال
   مختلفة عن اللي حطّيته → طابقتها بالظبط.
3. `supplier_balances` معرّفة `--surface` كرمادي فاتح مش أبيض، فتحويل
   خلفياتها البيضا لـ`var(--surface)` غيّر شكلها → رجّعتها لـ`var(--white)`.

### دَين متبقّي
لسه فيه **~2400 لون مكتوب صريح** جوه أنماط الصفحات (نصوص، حدود، شارات).
دي مابتستجبش للوضع الداكن، فبعض الشاشات فيها بقع فاتحة (411 عنصر في 33
شاشة، أكترها `sms.html` بـ221). تحويلها كله شغل كبير ومخاطرة بصرية —
الأفضل يتعمل شاشة‑بشاشة عند الحاجة، بنفس طريقة المقارنة دي.

**مراجعة المعمارية خلصت — الخطوات 1 إلى 7 كلها منشورة.**

---

## إصلاح النسخ الاحتياطي + تسريب توكن  ✅ 2026-09-04

### 1. تسريب سر على GitHub (غلطة مني)
`TRIGGER_TOKEN` بتاع `db-backup` كان مكتوب صريح في ملف الدالة اللي
صدّرته ورفعته على **ريبو عام** — مقروء لأي حد على الإنترنت، وبيسمح
بتشغيل نسخة احتياطية كاملة للقاعدة.

**اتصلح:**
- التوكن اتشال من الملف واتنشر فورًا.
- **اتغيّر** — الشيل من الملف مش كفاية، الـgit history شايل القديم.
- الجديد في `vault`، والدالة بتقراه عبر `public.vault_secret()` —
  `SECURITY DEFINER` صلاحيتها لـ`service_role` بس (اتسحب `EXECUTE` من
  `PUBLIC` صراحة، وإلا أي حد بمفتاح anon يقرا كل الأسرار).
- الـcron مابقاش فيه التوكن — بيقراه من vault وقت التشغيل، فتغييره
  بعد كده = `UPDATE` واحد من غير أي نشر.
- **اتأكدت: التوكن القديم بقى مرفوض (401).**

**حاجز ضد التكرار:** `scripts/check-secrets.js` + `.githooks/pre-commit`.
مختبر في الاتجاهين (ملف فيه سرّين → رفض بكود 1؛ الريبو النضيف → مرور).

### 2. النسخ الاحتياطي واقف من 28 يوليو (38 يوم)
**ليه محدّش خد باله:** الـcron بيقول «نجح» يوميًا — بس `net.http_post`
بيرجّع فورًا، فالنجاح ده **للإرسال مش للنتيجة**. النتيجة الحقيقية كانت
`546 WORKER_RESOURCE_LIMIT`.

**السبب:** الدالة بتحمّل كل جدول **كامل** في مصفوفة JS + بتبني أرشيف
للقاعدة كلها في بَفَر واحد. القاعدة بقت ~400MB (`sales_items` 108MB).

**الحل:** نسخة على دفعات بحالة محفوظة في `backup_runs`:
- كل نداء يشتغل بحد أقصى 30 جزء / 25 ثانية ويسيب مكانه؛ الـcron
  (`db-backup-resume`, كل دقيقة, `?resume=1`) بيكمّل.
- الذاكرة محدودة بصفحة واحدة (1000 صف) مهما كبرت القاعدة.
- كل صفحة ملف مستقل: `<folder>/<table>/part-NNNN.json.gz`

**مشكلتين ظهروا في التشغيل الفعلي:**
1. الحفظ كان في **آخر** النداء بس → الدالة تقع قبل ما تحفظ، فالنداء
   اللي بعده يعيد نفس الشغل ويقع تاني = **حلقة لا نهائية** (وقفت عند
   `order_logs`). الحل: حفظ كل 10 أجزاء.
2. ⚠️ غيّرت حجم الصفحة (500→1000) **وسط تشغيلة شغّالة**. الصفحة محفوظة
   كعدّاد، فالاستئناف بالحجم الجديد بيقرا من مكان غلط ويتخطى ~32 ألف
   صف **من غير أي خطأ يبان**. الحل: عمود `page_size` بيتثبّت مع
   التشغيلة. التشغيلة الناقصة اتعلّمت `failed` واتبدلت بنضيفة.

### النتيجة المتحقّق منها
| | |
|---|---|
| الجداول | **115/115** |
| الصفوف | **881,563** |
| الملفات | 964 جزء + manifest |
| الحجم المضغوط | **45 MB** (من ~400MB خام) |
| الأخطاء | **صفر** |
| المدة | 31 دقيقة |

**إثبات الاكتمال** — عدد الأجزاء مطابق لعدد الصفوف بالظبط:
| الجدول | صفوف | أجزاء | ⌈صفوف/1000⌉ |
|---|---|---|---|
| `store_item_prices` | 152,127 | 153 | 153 ✅ |
| `order_logs` | 116,250 | 117 | 117 ✅ |
| `sales_items` | 103,522 | 104 | 104 ✅ |

وسياسة الاحتفاظ شغّالة: **30 مجلد بالظبط** (`KEEP_LAST = 30`).

### ملاحظة مهمة
المؤسسة على خطة **Pro** — Supabase عامل نسخ احتياطي يومي للقاعدة أصلًا.
اللي كان واقف هو **النسخة الإضافية** (احتفاظ أطول + نسخة مستقلة)، وهي
مهمة خصوصًا مع النقل لسيرفر خاص.

### أسرار تانية مكتوبة صريح (مش مكشوفة على النت، لكن لازم تتحل)
| المكان | السر |
|---|---|
| cron `eplus_sync_tick` | مفتاح مزامنة eplus |
| `trg_delivery_perf` / `trg_fail_perf` / `trg_trip_return_perf` / `sweep_unrated_perf` | نفس سر تطبيق السواقين |

دول في القاعدة مش الريبو، فمش مكشوفين — **بس هيتسرّبوا لحظة ما أي دالة
منهم تتصدّر للريبو**. سر السواقين موجود جوه الـAPK كمان، فتغييره محتاج
إصدار جديد للتطبيق.
