...
class DispatchEngine {
  ...
  async start() {
    ...
    await this.run();
    this.interval = setInterval(function(self) { self.run(); }, 30000, this);
  }
  ...
  async getPendingOrders() {
    ...
    orders = orders.filter(function(o) {
      return new Date(o.bill_date || o.created_at) <= cutoff;
    });
  ...
  async getAvailableDrivers() {
    ...
    var breaksRes = await this.db
      .from('driver_breaks')
      .select('driver_id')
      .eq('status', 'approved')
      .in('driver_id', driverIds);
    var breakIds = {};
    (breaksRes.data||[]).forEach(function(b){breakIds[b.driver_id]=true;});
  ...
  async assignOrder(order, drivers, routeMap, driverActiveTrip) {
    ...
    var eligible = drivers.filter(function(driver) {
      ...
      if (!trip) return true;
      // لو عنده رحلة → نفس خط السير بس
      if (trip.trip_id && trip.route_id && !regionRouteIds.includes(trip.route_id)) return false;
      // ما وصلش الحد الأقصى
